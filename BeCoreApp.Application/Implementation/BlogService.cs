...
namespace BeCoreApp.Application.Implementation
{
    public class BlogService : IBlogService
    {
        ...
        {
            var query = _blogRepository.FindAll(x => x.HomeFlag == true).OrderByDescending(x => x.DateModified)
                .Select(x => new BlogViewModel
                {
                    Id = x.Id,
                    Name = x.Name,
        ...
        {
            var query = _blogRepository.FindAll(x => x.BlogCategory, a => a.BlogTags);
            ...
            var data = query.OrderByDescending(x => x.Id).Skip((pageIndex - 1) * pageSize).Take(pageSize)
                .Select(x => new BlogViewModel
                {
                    ...
                    Video = x.Video,
                    MildContent = x.MildContent,
                    ReferralLink = x.ReferralLink,
                    ReferralLinkRule = x.ReferralLinkRule,
                    HomeFlag = x.HomeFlag,
                    HotFlag = x.HotFlag,
                    Status = x.Status,
                    SeoPageTitle = x.SeoPageTitle,
