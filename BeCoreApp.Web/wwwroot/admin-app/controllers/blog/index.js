var BlogController = function () {
    ...
    function validateData() {
        ...
        dataBlog = {
            ...
            ReferralLinkRule: $('#txtReferralLinkRule').val(),
            Image: $('#txtImage').val(),
            Tags: $('#ddlTags').val(),
    ...
    function saveBlog(e) {
        ...
        if (validateData()) {
            ...
            $.ajax({
                ...
                success: function () {
                    be.notify('Cập nhật bài viết thành công', "", 'success');
    ...
    function deleteBlog(e, element) {
        ...
        be.confirm('Are you sure to delete?', function () {
            $.ajax({
                ...
                success: function () {
                    be.notify('Xóa bài viết thành công', "", 'success');
                    be.stopLoading();
                    loadData();
                },
                error: function (message) {
    ...
    function loadDetails(e, id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#txtReferralLink').val(response.ReferralLink);
                $('#txtReferralLinkRule').val(response.ReferralLinkRule);
                $('#txtImage').val(response.Image);
                $('#txtDescription').val(response.Description);
                $('#txtVideo').val(response.Video);
                $('#txtComment').val(response.Comment);
                $('#txtShare').val(response.Share);
                $('#txtLike').val(response.Like);
