var ProvinceController = function () {
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) { saveProvince(e) });

        $('body').on('click', '.btn-delete', function (e) { deleteProvince(e, this) });
    };


    function saveProvince(e) {
        e.preventDefault();

        var provinceVm = new ProvinceViewModel();
        if (provinceVm.Validate()) {
            $.ajax({
                type: "POST",
                url: "/Admin/Province/SaveEntity",
                data: { provinceVm },
                dataType: "json",
                beforeSend: function () {
                    be.startLoading();
                ...
                success: function () {
                    ...
                    be.notify('Cập Nhật tỉnh thành công',"", 'success');
    ...
    function deleteProvince(e, element) {
        e.preventDefault();
        be.confirm('Are you sure to delete?', function () {
            $.ajax({
                ...
                success: function () {
                    be.notify('Xóa tỉnh thành công',"", 'success');
                    be.stopLoading();
                    loadData(true);
                },
                error: function (message) {
    ...
    function loadData(isPageChanged) {
        ...
        $.ajax({
            type: "GET",
            url: "/admin/Province/GetAllPaging",
            data: {
