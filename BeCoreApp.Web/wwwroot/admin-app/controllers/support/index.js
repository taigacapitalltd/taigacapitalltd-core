var SupportController = function () {
    ...
    function validateData() {
        ...
        dataSupport = {
            Id: +($('#hidId').val()),
            Name: $('#txtName').val(),
            RequestContent: CKEDITOR.instances.txtRequestContent.getData()
        }
        ...
        if (!dataSupport.RequestContent)
            isValid = be.notify('Nội dung không được bỏ trống!!!', "", 'error');

        return isValid;
    }

    function registerControls() {
        CKEDITOR.replace('txtRequestContent', {});
        CKEDITOR.replace('txtRequestContentView', {});
        CKEDITOR.replace('txtResponseContentView', {});
    ...
    function saveSupport(e) {
        ...
        if (validateData()) {
            ...
            $.ajax({
                ...
                success: function () {
                    be.notify('Gửi hỗ trợ thành công', "", 'success');
    ...
    function loadDetails(e, id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                CKEDITOR.instances.txtRequestContentView.setData(response.RequestContent);
                CKEDITOR.instances.txtResponseContentView.setData(response.ResponseContent);
    ...
    function resetFormMaintainance() {
        $('#hidId').val(0);
        $('#txtName').val('');
        CKEDITOR.instances.txtRequestContent.setData('');
    }
