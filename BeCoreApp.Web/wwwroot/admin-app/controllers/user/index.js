var UserController = function () {
    ...
    function validateData() {
        ...
        if (!dataModel.FullName)
            isValid = be.notify('Họ và tên không được bỏ trống!!!', "", 'error');

        if (!dataModel.UserName)
            isValid = be.notify('Tên đăng nhập không được bỏ trống!!!', "", 'error');

        if (!dataModel.Email)
            isValid = be.notify('Email đăng nhập không được bỏ trống!!!', "", 'error');

        if (!dataModel.Password)
            isValid = be.notify('Mật khẩu không được bỏ trống!!!', "", 'error');

        if (dataModel.ConfirmPassword != dataModel.Password)
            isValid = be.notify('Mật khẩu và xác nhận mật khẩu không giống nhau!!!', "", 'error');

        if (!dataModel.RoleId) {
            isValid = be.notify('Vai trò không được bỏ trống!!!', "", 'error');
        }
        else {
    ...
    function saveData(e) {
        ...
        if (validateData()) {
            $.ajax({
                ...
                success: function () {
                    debugger;
                    be.notify('Cập Nhật thành viên thành công', '', 'success');
    ...
    function deleteUser(e, element) {
        ...
        be.confirm('Are you sure to delete?', function () {
            $.ajax({
                ...
                success: function () {
                    be.notify('Xóa thành viên thành công', '', 'success');
                    be.stopLoading();
                    loadData(true);
                },
                error: function (message) {
