var DirectionController = function () {
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) { saveDirection(e) });

        $('body').on('click', '.btn-delete', function (e) { deleteDirection(e, this) });
    };


    function saveDirection(e) {
        e.preventDefault();

        var directionVm = new DirectionViewModel();
        if (directionVm.Validate()) {
            $.ajax({
                type: "POST",
                url: "/Admin/Direction/SaveEntity",
                data: { directionVm },
                dataType: "json",
                beforeSend: function () {
                    be.startLoading();
                ...
                success: function () {
                    ...
                    be.notify('Cập Nhật hướng nhà thành công',"", 'success');
    ...
    function deleteDirection(e, element) {
        ...
        be.confirm('Are you sure to delete?', function () {
            $.ajax({
                ...
                success: function () {
                    be.notify('Xóa hướng nhà thành công',"", 'success');
                    be.stopLoading();
                    loadData(true);
                },
                error: function (message) {
    ...
    function loadData(isPageChanged) {
        ...
        $.ajax({
            type: "GET",
            url: "/admin/Direction/GetAllPaging",
            data: {
