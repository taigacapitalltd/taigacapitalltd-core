var ProjectCategoryController = function () {
    ...
    class ProjectCategoryViewModel {
        constructor() {
            this.Id = +($('#hidIdM').val());;
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) { saveProjectCategory(e) });

        $('body').on('click', '.btn-delete', function (e) { deleteProjectCategory(e, this) });
    };
    ...
    function saveProjectCategory(e) {
        ...
        if (projectCategoryVm.Validate()) {
            $.ajax({
                type: "POST",
                url: "/Admin/ProjectCategory/SaveEntity",
                data: { projectCategoryVm },
                dataType: "json",
                beforeSend: function () {
                    be.startLoading();
                },
                success: function () {
                    ...
                    be.notify('Cập Nhật loại dự án thành công',"", 'success');
    ...
    function deleteProjectCategory(e, element) {
        e.preventDefault();
        be.confirm('Are you sure to delete?', function () {
            $.ajax({
                ...
                success: function () {
                    be.notify('Xóa loại dự án thành công',"", 'success');
                    be.stopLoading();
                    loadData(true);
                },
                error: function (message) {
    ...
    function loadData(isPageChanged) {
        ...
        $.ajax({
            type: "GET",
            url: "/admin/ProjectCategory/GetAllPaging",
            data: {
    ...
    function loadDetails(e, element) {
        ...
        $.ajax({
            type: "GET",
            url: "/Admin/ProjectCategory/GetById",
            data: { id: $(element).data('id') },
            dataType: "json",
            beforeSend: function () {
                be.startLoading();
