var functionController = function () {
    ...
    function ValidateData() {
        ...
        var isValid = true;
        if (!data.Id) {
            be.notify('Root ID không được bỏ trống!!!', "", 'error');
            isValid = false;
        }
        if (!data.Name) {
        ...
        if (!data.ParentId) {
            be.notify('Parent ID không được bỏ trống!!!', "", 'error');
            isValid = false;
        }
        else {
        ...
        if (!data.Url) {
            be.notify('Url không được bỏ trống!!!', "", 'error');
            isValid = false;
        }
        ...
        if (isValid) {
            if (data.Action == 0) {
                checkIsExistId(data.Id, function (response) {
                    if (response) {
                        be.notify('Root ID đã tồn tại trong hệ thống!!!', "", 'error');
                    }
                    else {
                        createData(data);
                    }
    ...
    function createData(data) {
        $.ajax({
            ...
            success: function (response) {
                ...
                be.notify('Thêm chức năng thành công!!!', "", 'success');
    ...
    function removeData(id, text) {
        ...
        be.confirm('Bạn muốn xóa ' + text + '?', function () {
            $.ajax({
                type: "POST",
                ...
                success: function (response) {
                    ...
                    be.notify('Xóa thành công!!!!', '', 'success');
                    be.stopLoading();
    ...
    function updateParentId(data) {
        $.ajax({
            ...
            beforeSend: function () { },
            success: function (response) {
                be.notify('Cập nhật Parent ID thành công!!!!', '', 'success');
