...
namespace BeCoreApp.Areas.Admin.Controllers
{
    public class UploadController : BaseController
    {
        ...
        public async Task UploadImageForCKEditor(IList<IFormFile> upload, string CKEditorFuncNum, string CKEditor, string langCode)
        {
            ...
            if (upload.Count == 0)
            {
                await HttpContext.Response.WriteAsync("<script>window.parent.CKEDITOR.tools.callFunction(" + CKEditorFuncNum + ",'','vui lòng chọn một ảnh');</script>");
            }
            else
            {
                var file = upload[0];
        ...
        private GenericResult IsValidImage(IFormFile file)
        {
            if (file.Length > 3000000)
                return GenericResult.ToFail("Vui lòng upload hình ảnh dưới 3MB");
            ...
            if (!fileExtension.Equals(".jpg", ".png", ".PNG", ".jpeg") && !file.ContentType.Equals("image/jpg", "image/jpeg", "image/pjpeg", "image/png", "image/x-png"))
                return GenericResult.ToFail("Chỉ cho phép hình ảnh có định dạng là jpg hoặc png");
